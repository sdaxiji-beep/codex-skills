...
$realProject = (Get-Content (Join-Path $repoRoot 'deploy-config.json') -Raw | ConvertFrom-Json).projectRoot
...
try {
    ...
    # A: success
    $a = Invoke-McpPreviewProject `
        -ProjectPath $sandboxProject `
        -RequireConfirm $true `
        -RequestId 'case-a-success'
    ...
    # B: fast-gate fail -> denied + fast_gate_failed
    $env:MCP_PREVIEW_FORCE_FAST_GATE_FAIL = '1'
    $b = Invoke-McpPreviewProject `
        -ProjectPath $sandboxProject `
        -RequireConfirm $true `
        -RequestId 'case-b-fast-gate-fail'
    ...
    # C: non-sandbox path -> denied + path_not_allowed
    $c = Invoke-McpPreviewProject `
        -ProjectPath 'C:\not-allowed' `
        -RequireConfirm $true `
        -RequestId 'case-c-path-deny'
    ...
    # D: require_confirm=false -> denied + confirmation_required
    $d = Invoke-McpPreviewProject `
        -ProjectPath $sandboxProject `
        -RequireConfirm $false `
        -RequestId 'case-d-confirmation-deny'
    ...
    # E: preview command fail -> failed + structured error
    $env:MCP_PREVIEW_FORCE_PREVIEW_FAIL = '1'
    $e = Invoke-McpPreviewProject `
        -ProjectPath $sandboxProject `
        -RequireConfirm $true `
        -RequestId 'case-e-preview-fail'
    ...
    # F: real project allowlist path -> success
    $f = Invoke-McpPreviewProject `
        -ProjectPath $realProject `
        -RequireConfirm $true `
        -RequestId 'case-f-real-project-success'
