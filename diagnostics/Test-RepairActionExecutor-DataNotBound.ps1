...
$root = Join-Path ([System.IO.Path]::GetTempPath()) ("repair-data-not-bound-" + [guid]::NewGuid().ToString("N"))
...
try {
  ...
  if ($second.status -ne 'blocked' -or $second.reason -ne 'data_key_already_present') {
    throw 'second data_not_bound attempt should block as already present'
  }

  [pscustomobject]@{
    test = 'repair-action-executor-data-not-bound'
    pass = $true
    exit_code = 0
